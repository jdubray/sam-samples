    ...
    return {
        ...
        */
        init(present: (data: any) => void) {
            this.todo = todo ;
            this.todo.present = present ;
            //this.intents =  [ this.intents, {todo: todo.intents} ].reduce(Object.assign, {}); // 
            this.intents = Object.assign(this.intents, {todo: todo.intents} ) ;
        ...
        dispatch (event: any, present?: (data: any) => void) {
            ...
            props.forEach( (intent) => {
                if (event.__event === intent) {
                    let fqi = intent.split(".");
                    switch(fqi.length) {
                        case 1 : this[intent](event,present) ;
                    }
                }
                console.log(intent) ;
            }) ;
        ...
        trafficchartNeedsRefresh(data: any, present?: (data: any) => void) {
                ...
                let _data : any = {trafficChart: _services['TrafficChart'].getData()} ;
                console.log(_data) ;
                present(_data) ;
                return false ;
        },
        ...
        calendarNeedsRefresh(data: any, present?: (data: any) => void) {
                ...
                let _data : any = {calendar: _services['Calendar'].getData()} ;
                present(_data) ;
                return false ;
        },
        ...
        todoNeedsRefresh(data: any, present?: (data: any) => void) {
                ...
                let _data : any = {todoList: _services['ToDo'].getTodoList()} ;
                present(_data) ;
                return false ;
        },
        ...
        usersMappNeedsRefresh(data: any, present?: (data: any) => void) {
                ...
                let _data : any = {usersMap: _services['UsersMap'].getData()} ;
                present(_data) ;
                return false ;
        }
