...
public class DesiredStep<T extends Type> extends Step<T> {

	protected boolean _reached ;

	public DesiredStep(State from, Action action, State to, T results)
			throws Exception {
		super(from, action, to, results);
		this._reached = false ;
	}

	public DesiredStep(String id, State from, Action action, State to, T results)
			throws Exception {
	...
	public boolean isDesired(Step<T> s) {
